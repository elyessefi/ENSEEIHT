...
void create(string *chaine_dest, char *chaine_src){
    chaine_dest->str = malloc(sizeof(char*));
    chaine_dest->N = 0;
    for (int i = 0; chaine_src[i]!='\0'; i++){
...
void add(string *chaine, char c){
    char *inter = realloc(chaine->str, (chaine->N+1)*sizeof(char));
    if (inter){
        inter[chaine->N-1] = c;
        inter[chaine->N] = '\0';
        chaine->str = inter;
...
void delete(string *chaine, int i){
    int p ,k;
    for(k=0;chaine->str[k] != '\0';k++)
    {
        if (chaine->str[k] == chaine->str[i])
        {
            for(p=k; chaine->str[p]!='\0'; p++)
            chaine->str[p]=chaine->str[p+1];
        }
    }
    chaine->N --;
}
...
void destroy(string *chaine){
    chaine->str = realloc(chaine->str, 0);
    //free(chaine);
    //chaine = NULL;
}
